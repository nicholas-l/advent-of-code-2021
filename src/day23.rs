...
fn get_posible_positions(
    ...
    if let Some(Position::Room(room)) = map.get(start_pos) {
        let is_top_room = matches!(
            map.get(&(start_pos.0 + 1, start_pos.1)),
            Some(Position::Room(_))
        );
        let bottom_room_same = state
            .get(&(start_pos.0 + 1, start_pos.1))
            .map(|amp2| amp2 == amp)
            .unwrap_or(false);
        let ok = (is_top_room && bottom_room_same) || !is_top_room;
        if amp.is_room(room) && ok {
            return vec![];
        }
    }
    ...
    let mut see_positions = HashSet::new();

    while let Some((pos, cost)) = stack.pop() {
        // Get next free positions
        for dir in &dirs {
            let next_pos = (pos.0 + dir.0, pos.1 + dir.1);
            if see_positions.contains(&next_pos) {
                continue;
            }
            see_positions.insert(next_pos);
            ...
            if let Some(next_pos_value) = map.get(&next_pos) {
                match (next_pos_value, next_pos_state) {
                    ...
                    (Position::Room(room), None) => {
                        // Do I need to consider if already in final room?
                        if !is_in_room && amp.is_room(room) {
                            // If we are at the top of the room and there is available space below us then move to that instead
                            // Assumptions: Two positions in the room.
                            let is_room_below = matches!(
                                map.get(&(next_pos.0 + 1, next_pos.1)),
                                Some(Position::Room(_))
                            );
                            let is_occupied_below =
                                state.get(&(next_pos.0 + 1, next_pos.1)).is_some();
                            let bottom_room_same = state
                                .get(&(next_pos.0 + 1, next_pos.1))
                                .map(|amp2| amp2 == amp)
                                .unwrap_or(false);

                            if is_room_below && !is_occupied_below {
                                possible_positions
                                    .push(((next_pos.0 + 1, next_pos.1), cost + amp.cost() * 2));
                            } else if is_room_below && bottom_room_same {
                                possible_positions.push((next_pos, cost + amp.cost()));
                            } else if !is_room_below {
                                possible_positions.push((next_pos, cost + amp.cost()));
                            }
                        }
                        if is_in_room && !amp.is_room(room) {
...
pub fn star_one(input: impl BufRead) -> usize {
    ...
    while let Some(state) = stack.pop() {
        ...
        // println!("Completed {} states", stack.len());
    }

    best_state.unwrap().cost
}
